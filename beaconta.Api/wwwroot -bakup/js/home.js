...
$(function () {
    ...
    // ======= MENU (مضمّن هنا بدل menu.js) =======
    // const MENU = { ... كله احذفه }
    let MENU = {}; // سنعبئه من السيرفر
    ...
    async function loadMenu() {
        try {
            const res = await fetch("/api/menu/my", {
                headers: {
                    "Authorization": "Bearer " + localStorage.getItem("token"), // 🔑 التوكن بعد تسجيل الدخول
                    "Accept": "application/json"
                }
            });
            if (!res.ok) throw new Error("فشل تحميل القائمة");
            MENU = await res.json();
            // بعد التحميل ابني السايدبار
    ...
    await loadMenu(); // ✅ تحميل القوائم من الـ API
